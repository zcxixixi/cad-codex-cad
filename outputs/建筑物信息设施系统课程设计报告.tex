...
\node[box,right=of net,fill=red!10] (fw) {出口路由器\\下一代防火墙};
\node[core,right=of fw] (core) {双核心交换机\\2$\times$10 Gbit/s互联};
...
\draw[arr] (net)--node[above]{双链路}(fw);
\draw[arr] (fw)--node[above]{10 Gbit/s}(core);
\draw[arr] (core)--node[sloped,above]{OS2}(b1);
\draw[arr] (core)--node[right]{OS2}(b2);
\draw[arr] (core)--node[sloped,above]{OS2}(b3);
\draw[arr] (b1)--(a1);
\draw[arr] (b2)--(a2);
\draw[arr] (b3)--(a3);
\end{tikzpicture}
...
永久链路长度控制在90 m以内，信道长度不超过100 m。水平电缆沿金属弱电桥架敷设，出桥架后1--2根穿JDG20管，3--4根穿JDG25管暗敷。
...
\label{eq:cable}
\end{equation}
式中，$C$为本层电缆总长度；$F$、$N$分别为配线间至最远和最近信息点距离；$L$为端接余量，取2 m；$n$为信息模块数量；10\%为施工和路径修正余量。
...
语音主干按有语音业务的FD设置25对3类大对数电缆，并预留扩容，共配置20根。建筑间主干采用室外单模光缆，室内段采用低烟无卤阻燃护套。
